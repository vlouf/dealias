...
class Dealias:
    ...
    def initialize(self):
        ...
        dealias_vel, flag_vel = initialisation.initialize_unfolding(
            self.r, self.azimuth, azi_start_pos, azi_end_pos, self.velocity, flag_vel, vnyq=self.nyquist
        )
        vel = self.velocity.copy()
        vel[azi_start_pos, :] = dealias_vel[azi_start_pos, :]
        dealias_vel, flag_vel = initialisation.first_pass(
            azi_start_pos, vel, dealias_vel, flag_vel, self.nyquist, self.vshift, 0.75 * self.nyquist
        )
